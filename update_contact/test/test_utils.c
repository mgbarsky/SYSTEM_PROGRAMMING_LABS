...
enum status t_assert_array_double (double *received, double *expected, 
	...
	for (i=0; i<arr_size; i++){
		if (received == NULL){ //array points to NULL
			snprintf(err_buffer, MAX_ERR_LENGTH, 
					"EXPECTED value %f at position %d, GOT NULL pointer instead", 
					expected[i], i);
				return FAIL;	
		}
		else {
...
enum status t_assert_array (int *received, int *expected, 
	...
	if (received == NULL){ //array points to NULL
		snprintf(err_buffer, MAX_ERR_LENGTH, 
				"EXPECTED value %d at position 0, GOT NULL pointer instead", 
				expected[0]);
			return FAIL;	
	}
	for (i=0; i<arr_size; i++){
...
enum status t_assert_array_strings (char **received, char **expected, 
								int arr_size, char * err_buffer){
	int i;
	if (received == NULL){ //array points to NULL
		snprintf(err_buffer, MAX_ERR_LENGTH, 
				"EXPECTED value %s at position 0, GOT NULL pointer instead", 
				expected[0]);
			return FAIL;	
	}
	for (i=0; i<arr_size; i++){
		if (strcmp(received[i] , expected [i])!=0){
			snprintf(err_buffer, MAX_ERR_LENGTH, 
				"EXPECTED value %s at position %d, GOT %s instead", 
				expected[i], i, received[i]);
			return FAIL;	
		}		
	}
